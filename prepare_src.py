...
"""
ungoogled-chromium source preparation script for Microsoft Windows
Downloads, unpacks, prunes, patches, and applies domain substitution only.
...
def main():
    ...
    parser.add_argument(
        '--x86',
        action='store_true',
        help='Prepare source for x86 build')
    parser.add_argument(
        '--arm',
        action='store_true',
        help='Prepare source for ARM64 build')
    parser.add_argument(
        '--tarball',
        action='store_true',
        help='Use chromium tarball instead of cloning')
    ...
    if args.tarball:
        ...
        # Clone sources
        get_logger().info('Cloning sources...')
        subprocess.run(
            [sys.executable,
    ...
    get_logger().info('Downloading required Windows files...')
    ...
    if ESBUILD.exists():
        ...
        ESBUILD.mkdir()
    get_logger().info('Unpacking Windows downloads...')
    ...
    # Substitute domains
    get_logger().info('Applying domain substitution...')
    ...
    get_logger().info('Done! Patched source tree is ready at: %s', source_tree)
    get_logger().info('You can now create your custom patches from build/src')
