...
# copy only the fingerprint-chromium folder into patches/
if [ ! -d "$BLUTVINE_DIR/fingerprint-chromium" ]; then
    echo "ERROR: fingerprint-chromium folder not found in $BLUTVINE_DIR"
    exit 1
fi
cp -r "$BLUTVINE_DIR/fingerprint-chromium" "$PATCHES_DIR/"
echo "Copied fingerprint-chromium into patches/"

# append BlutVine series entries to existing series file (do not replace)
