...
DOMRectList* Element::getClientRects() {
  ...
  for (auto& rect : rects) {
    GetDocument().AdjustRectForScrollAndAbsoluteZoom(rect, *element_layout_object);
  }
...
Vector<gfx::RectF> Element::GetClientRectsNoAdjustment() {
  ...
  if (command_line->HasSwitch(switches::kFingerprint) && !ElementHasTransform(*this)) {
    for (auto& quad : quads) {
